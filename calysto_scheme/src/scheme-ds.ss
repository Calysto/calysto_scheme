    ...
    (let ((apair1 (car fields))
      ...
      (if (not value)
          ...
          (instantiate^
            ...
            value
            (cdr^ apair1)
            (make-cont2 <cont2-123> apair2 pair2 value k))))))
  ...
  (lambda (value1 value2 fields)
    (let ((apair2 (car fields))
      ...
      (instantiate^
        ...
        s-car
        (cdr^ apair2)
        (make-cont2 <cont2-122> value2 value1 s-car k)))))
  ...
  (lambda (value1 value2 fields)
    (let ((ap (car fields))
      ...
      (instantiate^
        ...
        s
        (cdr^ ap)
        (make-cont2 <cont2-124> value1 value2 ap k2)))))
    ...
    (cond
      ...
      ((pair? pattern)
       (instantiate^
         ...
         s
         (car^ ap)
         (make-cont2 <cont2-125> ap pattern s k2)))
...
(define*
  ...
  (lambda (s var avar k2)
    (record-case (cdr s)
      ...
      (composite (s1 s2)
       (apply-sub^ s1 var avar (make-cont2 <cont2-126> s2 k2)))
...
(define-native
  ...
  (lambda (args env2 info handler fail k2)
    (let* ((op-prim (car args))
      ...
      (apply-cont2 k2 (sort op (cadr args)) fail))))
...
(if (string=? (host-environment-native) "scheme") (start))
