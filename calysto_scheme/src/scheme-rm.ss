...
(define <cont-57>
  (lambda (apair1 apair2 pair1 pair2 k)
    (if (not value_reg)
        ...
        (begin
          (set! k2_reg
            (make-cont2 <cont2-123> apair2 pair2 value_reg k))
...
(define <cont2-123>
  (lambda (apair2 pair2 s-car k)
    (set! k2_reg
      (make-cont2 <cont2-122> value2_reg value1_reg s-car k))
...
(define <cont2-125>
  (lambda (ap pattern s k2)
    (set! k2_reg
      (make-cont2 <cont2-124> value1_reg value2_reg ap k2))
    (set! ap_reg (cdr^ ap))
    (set! s_reg s)
    (set! pattern_reg (cdr pattern))
    (set! pc instantiate^)))
  ...
  (lambda ()
    (if (not (length-two? args_reg))
        (begin
          ...
          (set! pc runtime-error))
        (return*
          (sort-native args_reg env2_reg info_reg handler_reg fail_reg
            k2_reg)))))
    ...
    (if (constant? pattern_reg)
        ...
        (if (pattern-variable? pattern_reg)
            ...
            (if (pair? pattern_reg)
                (begin
                  (set! k2_reg
                    (make-cont2 <cont2-125> ap_reg pattern_reg s_reg k2_reg))
          ...
          (if (eq? (car temp_1) 'unit)
              ...
              (if (eq? (car temp_1) 'composite)
                  (let ((s1 'undefined) (s2 'undefined))
                    ...
                    (set! k2_reg (make-cont2 <cont2-126> s2 k2_reg))
                    (set! s_reg s1)
                    (set! pc apply-sub^))
...
(define-native
  ...
  (lambda (args env2 info handler fail k2)
    (let ((op-prim 'undefined) (op 'undefined))
      (set! op-prim (car args))
      (set! op
        (lambda (v1 v2)
          (op-prim (list v1 v2) env2 info handler fail REP-k)))
      (set! value2_reg fail)
      (set! value1_reg (sort op (cadr args)))
      (set! k_reg k2)
      (set! pc apply-cont2))))
